    ...
    this.middleWare = [
        function parseJSON(socket, message) {
            try {
                ...
                throw new Error(sprintf('Cannot parse JSON message! Reason: %s', err.message));
            }
        }
    ];
    this.loadPlugins();
...
Wampify.prototype.useMiddleWare = function(socket, data) {
    if ((socket instanceof WebSocket) && data) {
        ...
        _.forEach(this.middleWare, function(use) {
            if (_.isFunction(use)) {
                data = use.apply(this, [socket, data]);
            }
        }, this);

        return this.parse.apply(this, [socket, data]);
    } else {
